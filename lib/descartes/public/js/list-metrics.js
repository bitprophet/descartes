...
// Query cache for metrics when button is pressed.
var queryForMetrics = function() {
  clearGraphs();
  $('#search.alert').alert('close');
  var mySearchString = $('input.search-ahead').val();
  myMatchedMetrics = [];
  $.ajax({
    accepts: {'json': 'application/json'},
    cache: false,
...
// Bind on Enter keypress
$('#metrics_graph_select ul li').on('keypress', 'input.search-ahead',
  function(e) { if (e.which === 13) { queryForMetrics(); }}
);
// Bind on button click
$(document).on('click', 'button.filter', function() { queryForMetrics(); });
