...
// Interpolate values from external data sources
var interpolateTarget = function(target) {
    // JS's regex API seems kinda stupid; using 'g' means you no longer get
    // captures in the result?
    var rgx = "#{([^}]+)}";
    var lRegex = new RegExp(rgx);
    var gRegex = new RegExp(rgx, 'g');
    var result = target;
    var matches = target.match(gRegex);
    // Short-circuit
    if (matches === null) {
        return target;
    }
    // Replace all expandos \o/
    matches.forEach(function(expando) {
        // Extract e.g. 'foo' from '#{foo}'. Bluh
        var identifier = expando.match(lRegex)[1];
        // Ask Clusto for the expansion
        $.ajax({
            async: false, // kinda really do want it to be sync
            url: clustoUrl + identifier + '.json',
            accepts: {json: 'application/json'},
            error: function(xhr, textStatus, errorThrown) { console.log(errorThrown); }
        // Perform the replacement by mutating result string
        }).done(function (d) {
            // JS array-o-strings like ['a','b','c'] => Graphite brace
            // expression like '{a,b,c}'.
            result = result.replace(expando, '{' + d.join() + '}')
        })
    });
    return result;
}
...
var constructGraphUrl = function(graph) {
  ...
  if (graph.options !== undefined) {
    ...
    // Default to hideLegend=true if not given. (E.g. metrics list page graph
    // preview already sets hideLegend=false)
    if (!myLocalGraphOptions.match(/hideLegend/)) {
        myLocalGraphOptions += "&hideLegend=true";
    }
  }
  ...
  for (var j in graph.targets) {
    if (graph.targets[j].axis === 2) {
      targets += '&target=secondYAxis(' + interpolateTarget(graph.targets[j].name) + ')';
    } else {
      targets += '&target=' + interpolateTarget(graph.targets[j].name);
    }
  }
